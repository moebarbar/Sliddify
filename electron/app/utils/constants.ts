...
const appDirectoryName = "Sliddify Open Source";
...
function getAppDataBaseDir(tempRoot: string): string {
  const home = getHomeDir();
  const fallback = path.join(tempRoot, "sliddify-app-data");
...
function setElectronPath(name: string, dir: string): void {
  try {
    ...
    console.warn(`[Sliddify] Failed to set Electron path ${name}=${dir}`, error);
  }
}
...
function appendDiskCacheSwitch(cacheDir: string): void {
  try {
    ...
    console.warn("[Sliddify] Failed to configure Chromium disk cache path", error);
  }
}
...
export function initializeAppPaths(): ElectronAppPaths {
  ...
  const userDataDir = firstWritableDirectory(
    unique([
      ...
      path.join(tempRoot, "sliddify-user-data"),
    ]),
    "user data"
  );
  ...
  const tempDir = firstWritableDirectory(
    unique([path.join(tempRoot, "sliddify"), path.join(userDataDir, "temp")]),
    "temporary"
  );
