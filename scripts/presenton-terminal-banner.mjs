/**
 * Docker / startup terminal banner for Sliddify.
...
function loadAsciiBanner() {
  ...
  const asciiPath = path.join(thisDir, "sliddify-ascii.txt");
 ...
 */
export function printSliddifyStartupBanner(opts = {}) {
  const nextPort = opts.nextPort ?? 3000;
  const fastapiPort = opts.fastapiPort ?? 8000;
  const hostHttpPort =
    opts.hostHttpPort ??
    process.env.SLIDDIFY_HOST_HTTP_PORT ??
    process.env.SLIDDIFY_PUBLIC_PORT ??
    "5000";
  ...
  const summary = [
    ...
    "",
    "   " + muted("Made with ❤️  by the Sliddify team"),
  ].join("\n");
