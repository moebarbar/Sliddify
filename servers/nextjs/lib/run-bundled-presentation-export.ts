...
export function getSliddifyAppRoot(): string {
  return (
    process.env.SLIDDIFY_APP_ROOT?.trim() ||
...
function extractSessionTokenFromCookieHeader(cookieHeader?: string): string | undefined {
  ...
  const match = cookieHeader.match(/(?:^|;\s*)sliddify_session=([^;]+)/);
  if (!match?.[1]) {
    return undefined;
  }
...
async function runBundledPresentationExportLocked(params: {
  ...
  const appRoot = getSliddifyAppRoot();
  ...
  const tempBase =
    process.env.TEMP_DIRECTORY?.trim() || path.join(os.tmpdir(), "sliddify");
